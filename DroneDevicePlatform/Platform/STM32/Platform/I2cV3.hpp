...
#ifndef PLATFORM_STM32_I2CV3_HPP_
#define PLATFORM_STM32_I2CV3_HPP_

#include <Platform/I2cDataTypeV2V3.hpp>
...
public:
	...
	I2c(I2CRate aRate, DataTimeSetup aDataSetupTime, DataTimeSetup aDataHoldTime, Callback aCallback = nullptr) :
		timingRegValue{getTimingRegValue(aRate, aDataSetupTime, aDataHoldTime)},
		callback{aCallback}
	{
		rcc_periph_clock_enable(kClockBranch);
		i2c_enable_analog_filter(kPeriph);
		I2C_TIMINGR(kPeriph) = timingRegValue;
...
private:
	...
	//! @brief I2C timing register value request
	//! @param[in] aRate - I2C bus rate
	//! @param[in] aDataSetupTime - data setup time
	//! @param[in] aDataHoldTime - data hold time
	//! @return I2C timing register value
	static uint32_t getTimingRegValue(I2CRate aRate, DataTimeSetup aDataSetupTime, DataTimeSetup aDataHoldTime)
	{
		I2CTimeDesc timeDesc{aRate, aDataHoldTime, aDataSetupTime, rcc_apb1_frequency / 1000};
		return timeDesc.getRegValue();
	}
};

#endif // PLATFORM_STM32_PLATFORM_I2CV3_HPP_
