...
#ifndef PLATFORM_STM32F0XX_DMA_HPP_
#define PLATFORM_STM32F0XX_DMA_HPP_
...
class DmaBase {
	...
	static_assert(channel >= 1 && channel <= 7, "Incorrect channel");
...
public:
	...
	{
		instance->handler(statusToFlags(aStatus));
	}
...
protected:
	...
	virtual void handler(uint32_t aFlags) = 0;
	...
	static constexpr uint8_t numberToIrq()
	{
		if (controller == 1) {
			if (channel == 1) {
				...
				return NVIC_DMA1_CHANNEL2_3_DMA2_CHANNEL1_2_IRQ;
			} else {
				return NVIC_DMA1_CHANNEL4_7_DMA2_CHANNEL3_5_IRQ;
			}
		} else {
			if (channel <= 2) {
				return NVIC_DMA1_CHANNEL2_3_DMA2_CHANNEL1_2_IRQ;
...
protected:
	void handler(uint32_t aFlags) override
	{
		callback(aFlags);
	}
...
#endif // PLATFORM_STM32F0XX_DMA_HPP_
